...
#the alien burps when I left click the mouse
words = c.create_text(200,280)
def mouth_open():
...
#the alien blinks when key a is pressed and reopen its eyes when key z is pressed
def blink2(event):
...
#moves the eyeball when the direction keys are pressed
